...
class Gui(object):
    def __init__(self, root):
        ...
        self.download_sfml_box = Checkbutton(root, text="SFML", variable=self.var1).grid(row=0)
        ...
        self.exit_button = Button(root, text="Exit", command=root.quit).grid(row=2, column=1)
        self.where_button = Button(root, text="Where?", command=self.ask_dir).grid(row=2, column=2)
